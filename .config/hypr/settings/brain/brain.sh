...
EOF
  ...
  echo
  opt1=$(gum choose --height=13 "monitor" "waybar " "blur" "corners" "animations" "window gaps" "backgrounds" "eye protection" "manual settings" "default apps" "exit")
  clear
  if [[ $opt1 == "exit" ]]; then
    break
  elif [[ $opt1 == "monitor" ]]; then
    clear
    cat <<"EOL"
 __  __             _ _
...
EOL
    ...
    echo
    opt2=$(gum file --height=6 ~/.config/hypr/settings/brain/config/monitor/)
    cp "$opt2" "$HOME/.config/hypr/settings/monitor.conf"
  elif [[ $opt1 == "waybar " ]]; then
    clear
    cat <<"EOL"
__        __          _
...
EOF
      echo "Press :'Enter' to confirm , 'Esc' to return back"
      echo
      lis1=$(ls "$HOME/.config/hypr/settings/brain/config/waybar_themes/")
      opt2=$(gum choose --height=10 $lis1)
      if [[ "$opt2" ]]; then
        echo "$opt2 applying "
        cp -r "$HOME/.config/hypr/setting/brain/config/waybar_themes/$opt2/"* "$HOME/.config/waybar/"
        if [[ "$opt2" == "default" ]]; then
...
EOF
      echo "Press :'Enter' to confirm , 'Esc' to return back"
      echo
      opt2=$(gum file --height=3 "$HOME/.config/settings/hypr/settings/brain/config/status_bar/")
...
EOF
    echo "Press :'Enter' to confirm , 'Esc' to return back"
    echo
    opt2=$(gum file --height=20 "$HOME/.config/hypr/settings/brain/config/blur_system/")
    cp "$opt2" "$HOME/.config/hypr/settings/blur_system.conf"
  elif [[ $opt1 == "eye protection" ]]; then
    clear
    cat <<"EOF"
                                   _            _   _
...
EOF
    echo "Press :'Enter' to confirm , 'Esc' to return back"
    echo
    opt2=$(gum file --height=5 ~/.config/hypr/settings/brain/config/eye_protection/)
    if [[ "$opt2" ]]; then
...
EOF
    echo "Press :'Enter' to confirm , 'Esc' to return back"
    echo
    opt2=$(gum file --height=3 ~/.config/hypr/settings/brain/config/corners/)
    if [[ "$opt2" ]]; then
      cp "$opt2" "$HOME/.config/hypr/settings/corners.conf"
    fi
  elif [[ $opt1 == "window gaps" ]]; then
    clear
    cat <<"EOF"
          _           _
...
EOF
    echo "Press :'Enter' to confirm , 'Esc' to return back"
    echo
    opt2=$(gum file --height=7 ~/.config/hypr/settings/brain/config/window_gaps/)
    if [[ "$opt2" ]]; then
      cp "$opt2" "$HOME/.config/hypr/settings/window_gaps.conf"
    fi
  elif [[ $opt1 == "backgrounds" ]]; then
    clear
    cat <<"EOF"
 ____             _                                   _
...
EOF
      ...
      if [[ $opt3 == "garuda wallpaper" ]]; then
        echo "Do you want to use garuda wallpapers instead of custom ? "
        gum confirm && opt4="y" || opt4="n"
        if [[ $opt4 == "y" ]]; then
          cp "$HOME/.config/hypr/settings/brain/config/background/scripts/slideshow_garuda.conf" "$HOME/.config/wpaperd/config.toml"
          cp "$HOME/.config/hypr/settings/brain/config/background/slideshow.conf" "$HOME/.config/hypr/settings/background.conf"
          ...
          echo "garuda" >"$HOME/.config/settings/hypr/settings/brain/scripts/wall_condition.txt"
        else
          gum spin --spinner meter --title "No changes applied" -- sleep 3
        fi
      elif [[ $opt3 == "custom wallpaper" ]]; then
        echo "Do you want to use custom wallpapers instead of garuda "
        gum confirm && opt4="y" || opt4="n"
        if [[ $opt4 == "y" ]]; then
          if [[ ! -d "$HOME/custom_wallpapers/" ]]; then
          ...
          if [[ ! -d "$HOME/custom_wallpapers/photos" ]]; then
            mkdir "$HOME/custom_wallpapers/photos"
            cp "/usr/share/wallpapers/garuda-wallpapers/"* "$HOME/custom_wallpapers/photos"
          fi
          cp "$HOME/.config/hypr/settings/brain/config/background/scripts/slideshow_custom.conf" "$HOME/.config/wpaperd/config.toml"
          cp "$HOME/.config/hypr/settings/brain/config/background/slideshow.conf" "$HOME/.config/hypr/settings/background.conf"
          read -rp "Enter time interval in minutes : " opt5
          echo "duration = '$opt5 m'" >>"$HOME/.config/wpaperd/config.toml"
          ...
          echo "custom" >"$HOME/.config/hypr/settings/brain/scripts/wall_condition.txt"
        else
          gum spin --spinner meter --title "No changes applied" -- sleep 3
        fi
      fi
    elif [[ $opt2 == "add wallpaper" ]]; then
      clear
      cat <<"EOF"
    _       _     _                    _ _
   / \   __| | __| |    __      ____ _| | |
...
EOF
      echo "Press :'Enter' to confirm , 'Esc' to return back"
      echo
      cond1=$(cat "$HOME/.config/hypr/settings/brain/scripts/wall_condition.txt")
      if [[ $cond1 == "garuda" ]]; then
      ...
      if [[ ! -d "$HOME/custom_wallpapers/animated" ]]; then
        mkdir "$HOME/custom_wallpapers/animated"
      fi
      if [[ $opt4 == "add photos" ]]; then
        echo "Navigate to the photo : "
        opt5=$(gum file --height=20 "$HOME")
        cp "$opt5" "$HOME/custom_wallpapers/photos/"
      elif [[ $opt4 == "add animated" ]]; then
...
EOF
      echo "Press :'Enter' to confirm , 'Esc' to return back"
      echo
      cond1=$(cat "$HOME/.config/settings/hypr/settings/brain/scripts/wall_condition.txt")
      if [[ $cond1 == "garuda" ]]; then
        gum spin --spinner meter --title "Your current path is set to garuda" -- sleep 3
        gum spin --spinner meter --title "please change it to custom from 'background path' options" -- sleep 4
      fi
      opt3=$(gum choose --height=5 "session static" "slideshow" "animated")
      if [[ "$opt3" == "slideshow" ]]; then
        cond1=$(cat "$HOME/.config/hypr/settings/settings/brain/scripts/wall_condition.txt")
        if [[ $cond1 == "garuda" ]]; then
          cp "$HOME/.config/hypr/settings/brain/config/background/scripts/slideshow_garuda.conf" "$HOME/.config/wpaperd/config.toml"
          cp "$HOME/.config/hypr/settings/brain/config/background/slideshow.conf" "$HOME/.config/hypr/settings/background.conf"
          read -rp "Enter time interval in minutes : " opt5
      ...
      elif [[ "$opt3" == "session static" ]]; then
        cond1=$(cat "$HOME/.config/settings/hypr/settings/brain/scripts/wall_condition.txt")
        if [[ $cond1 == "garuda" ]]; then
          cp "$HOME/.config/hypr/settings/brain/config/background/static_garuda.conf" "$HOME/.config/settings/hypr/settings/background.conf"
          gum spin --spinner meter --title "Press Mod+Shift+c to see the changes" -- sleep 3
        elif [[ $cond1 == "custom" ]]; then
          cp "$HOME/.config/hypr/settings/brain/config/background/static_custom.conf" "$HOME/.config/settings/hypr/settings/background.conf"
          gum spin --spinner meter --title "Please restart your system" -- sleep 2
          gum spin --spinner meter --title "Press Mod+Shift+c to change wallpapers" -- sleep 3
        fi
      elif [[ "$opt3" == "animated" ]]; then
        var1=$(ls "$HOME/custom_wallpapers/animated/" | wc -l) &>/dev/null
        if [[ -d "$HOME/custom_wallpapers/animated/" && "$var1" -ne 0 ]]; then
          cp "$HOME/.config/settings/hypr/settings/brain/config/background/animated.conf" "$HOME/.config/settings/hypr/settings/background.conf"
          gum spin --spinner meter --title "Please restart your system" -- sleep 2
          gum spin --spinner meter --title "Press Mod+Shift+c to change wallpapers" -- sleep 3
        else
  ...
  elif [[ $opt1 == "manual settings" ]]; then
    clear
    cat <<"EOF"
 __  __                            _   _   _
...
EOF
    echo "Press :'Enter' to confirm , 'Esc' to return back"
    echo
    opt2=$(gum file --height=4 ~/.config/settings/hypr/settings/brain/config/animations/)
    if [[ "$opt2" ]]; then
