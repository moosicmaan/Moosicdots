...
local function gen_shortcut(config)
	...
	for _, item in pairs(shortcut) do
		...
		api.nvim_buf_add_highlight(config.bufnr, 0, item.group or "DashboardShortCut", first_line, start, _end)
...
local function gen_center(plist, config)
	...
	for i = 3, plist_len do
		api.nvim_buf_add_highlight(config.bufnr, 0, "DashboardProjectIcon", first_line + i - 1, 0, start_col + 3)
		api.nvim_buf_add_highlight(config.bufnr, 0, "DashboardFiles", first_line + i - 1, start_col + 3, -1)
	...
	api.nvim_buf_add_highlight(config.bufnr, 0, "DashboardMruIcon", first_line + plist_len, 0, scol + #config.mru.icon)

	for i, data in pairs(mgroups) do
		local len, group = unpack(data)
		if group then
			api.nvim_buf_add_highlight(config.bufnr, 0, group, first_line + i + plist_len, start_col, start_col + len)
		end
		api.nvim_buf_add_highlight(config.bufnr, 0, "DashboardFiles", first_line + i + plist_len, start_col + len, -1)

		local text =
			api.nvim_buf_get_lines(config.bufnr, first_line + i + plist_len, first_line + i + plist_len + 1, false)[1]
...
local function theme_instance(config)
	...
	project_list(config, function(plist)
		...
		require("dashboard.theme.moosicheader").generate_header(config)
		...
		local size = math.floor(vim.o.lines / 2) - math.ceil(api.nvim_buf_line_count(config.bufnr) / 2) - 2
