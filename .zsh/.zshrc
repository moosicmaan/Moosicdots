...
# JDB ---> setting VIM mode for the commandline
set -o vi
# <--- JDB

# JDB ---> Fish likes to steal this...
SHELL=/usr/bin/zsh
# chsh -s $(which zsh)     #permantly changes $SHELL to zsh
# JDB --->
# set collortheme to current wallpaper
#cwp=$( cat $HOME/.cache/current_wallpaper )
#wal -i $cwp >/dev/null 2>&1

# JDB ---> Other variables
my_font=ter-v28n.psf.gz
#my_font=FiraMonoNerdFontPropo-Regular.otf
font_directory=/usr/share/kbd/consolefonts/
#font_directory=/usr/share/fonts/OTF/
d_line="\b===========================================================================\n"
s_line="\b${BWhite}---------------------------------------------------------------------------"
...
if [[  $DISPLAY < 1  ]];
  then
    #JDB --> custom prompt for tty sessions
    PS1=$'%F{red}┌─%f%F{yellow}in%f %F{magenta}%~%f\n%F{red}└─%f%F{yellow}►> %f'
    #JDB --> add the time on the right with a status check on the previous command
    RPROMPT="%(?.%F{#008000}√.%F{#ff0000}?%?)%f%F{#c0c0c0}%T%f"
    Console=true
  else
    #load starship, if installed and available
...
#JDB ---> Welcome
echo -n "${BWhite}"$d_line"\bWelcome "$USER" ;-)\n"$d_line
echo -n \
"\b----> mpg123 - For your entertainment                                 <----\n"\
"\b----> in kitty - kitten icat <file_name> - display images             <----\n"\
"\b---->            kitten diff - side-by-side comparison                <----\n"\
"\b----> cmatrix -bfs - red or blue?                                     <----\n"\ | lolcat
echo $s_line
if [[ $Console ]];
  then
    setfont $font_directory$my_font
    echo "\b---->${Yellow} Font set to:\t"$font_directory$my_font"\n"$s_line
  #else
	#echo '\n'
    #fastfetch --load-config MOOSICized &&
    # echo "\033[A"$s_line
    #echo $s_line
fi
echo -n \
"\b----> NOTE: use garuda-update instead of 'pacman -Syu'                <----\n"\
"\b----> some python/development stuff:                                  <----\n"\
"\b----> ptpython or ipython - interactive python shells                 <----\n"\
"\b----> nvim - no pain, no gain - glrnvim, graphical version            <----"\ | lolcat
echo -n "\b${BWhite}\n"$d_line"\bYour tty session: \t"
tty | lolcat
echo -n "${BWhite}Current date and time:\t"
date | lolcat
echo -n "${BWhite}Shell:                \t"
echo $SHELL | lolcat
echo -n "\b${BWhite}\n\033[A"$d_line
...
#JDB --> turning off this annoying features
